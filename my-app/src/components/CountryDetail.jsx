...
export default function CountryDetail() {     //both work     //any varaible name
 ...
 // console.log(params)            //{country: "canada"}            
  ...
  function updateCountryData(data) {
    ...
    data.data.objects[0].borders.map((border) => {                            //map return those number of borders which the api can return //example for germany .map() runs 9 times → 9 API calls → 9 border countries //example for france  .map() runs 8 times → 8 API calls → 8 border countries
      fetch(`https://api.restcountries.com/countries/v5/codes.alpha_3/${border}?pretty=1&api-key=rc_live_605ebf66a6544395a253e2d4832952c5`)
      .then((res) => res.json())
      .then((borderData) => {
        // console.log(borderData.data.objects[0].names.common)
